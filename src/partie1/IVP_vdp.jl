using Plots
using LinearAlgebra
"""
~gergaud/ENS/Control/ODE/Matlab/IVP_vdp.m

Auteurs:  Joseph GERGAUD
Date:     nov. 2005
Adresse:  INP-ENSEEIHT-IRIT-UMR CNRS 5055
        2, rue Camichel 31071 Toulouse FRANCE
Email:    gergaudenseeiht.fr
***************************************************************************

Int�gration de l'equation differentiel de l'equation de Van der Pol
ref: Hairer


Initialisations
---------------
""" 

include("ode_gauss_v3.jl")
include("ode_gauss_v2.jl")
include("ode_gauss.jl")
include("plot_sol.jl")
include("d_phi_vdp.jl")
include("phi_vdp.jl")
include("edo_euler.jl")
include("edo_runge.jl")
include("edo_rk4.jl")
include("edo_heun.jl")
#clear all close all
#nom_fich = "IVP_vdp_N_10.txt"
#if exist(nom_fich,"file")
#    delete(nom_fich)
#end
#format long
#diary(nom_fich)
#


y0 = [2.008619860874843136; 0]
t0 = 0
tf = 6.663286859323130189
N0 = [120:60:1080; 1200:600:10800 ]# 12000:12000:72000] # multiple de 12 pour avoir des nombres entier si on divise
options = zeros(3)
# par 3 ou 4
#
# Solutions y_1 et y_2 et plan de phase pour RKE
# ----------------------------------------------
N = 10
#println("Euler")
#println("-----")
pyplot()
plt=Plots.plot(layout=(1,3))
T,Y = ode_euler(phi_vdp,[t0 tf],y0,N)
#println("[T Y]=")
#println([T Y])
plot_sol(plt,T,Y,"blue",["euler","euler","euler"])
#println("Runge")
#println("-----")
T,Y = ode_runge(phi_vdp,[t0 tf],y0,N)
#println("[T Y]=")
#println([T Y])
plot_sol(plt,T,Y,"yellow",["runge","runge","runge"])
#println("Heun")
#println("----")
T,Y = ode_heun(phi_vdp,[t0 tf],y0,N)
#println("[T Y]=")
#println([T Y])
plot_sol(plt,T,Y,"magenta",["heun","heun","heun"])
#println("rk41")
#println("----")
T,Y = ode_rk4(phi_vdp,[t0 tf],y0,N)
#println("[T Y]=")
#println([T Y])
plot_sol(plt,T,Y,"green",["rk41","rk41","rk41"])
#println("rk42")
#println("-----")
T,Y = ode_rk4(phi_vdp,[t0 tf],y0,N)
#println("[T Y]=")
#println([T Y])
plot_sol(plt,T,Y,"cyan",["rk42","rk42","rk42"])
#println("Gauss, point fixe")
#println("-----------------")
options[1] = N
# options[2] = nbre max iteration point fixe
options[2] = 15
# epsilon pour le test du point fixe
# Il faut 1.e-12 car pour les valeurs de N grandes on atteint avec RK4 1.e-12 comme erreur absolue
options[3] = 1e-6
#println("[N, nb_itmax, f_eps]")
#println(options)
T,Y,nphie,ifail = ode_gauss_v2(phi_vdp,[t0 tf],y0,options)
#println("[T Y]=")
#println([T Y])
#nphie
#ifail
plot_sol(plt,T,Y,"red",["gauss_v2","gauss_v2","gauss_v2"])

#println("Gauss, Newton")
#println("-------------")
#println("[N, nb_itmax, f_eps]")
#println(options)
T,Y,nphie,ndphie,ifail = ode_gauss_v3(phi_vdp,d_phi_vdp,[t0 tf],y0,options)
#println("[T Y]=")
#println([T Y])
nphie
ndphie
ifail
plot_sol(plt,T,Y,"red",["gauss_v3","gauss_v3","gauss_v3"])

##legend("Euler", "Runge","Heun","RK4_1","RK4_2","gauss v2","gauss_Newton","Location","SouthEast")


#print -depsc fig_solutions_vdp
#format short
#diary
#
#pause

# Courbes d"ordre
# ---------------


N = N0
# Euler
err1 = zeros(length(N))
err2 = zeros(length(N))
plt=Plots.plot(layout=(1,2))
for i=1:length(N)
    T,Y = ode_euler(phi_vdp,[t0 tf],y0,N[i])
    n=length(T)
    err1[i] = abs(y0[1]-Y[n,1])
    err2[i] = abs(y0[2]-Y[n,2])
end
#subplot(1,2,1)
#loglog(N,err1, xlabel="fe", ylabel="erreur pour y_1")
i=round(length(N)/2)
#text(N[i],err1[i] , "Euler")
#subplot(1,2,2)
#loglog(N,err2, xlabel="fe", ylabel="erreur pour y_2")
#text(N[i],err2[i] , "Euler") 
#
#subplot(1,2,1)
Plots.plot!(log10.(N),log10.(err1), xlabel="log_{10}(fe)", ylabel="log_{10}(erreur pour y_1)",subplot=1)
i=round(length(N)/2)
#text(log10.(N[i]),log10.(err1[i] ) ,"Euler")
#subplot(1,2,2)
Plots.plot!(log10.(N),log10.(err2), xlabel="log_{10}(fe)", ylabel="log_{10}(erreur pour y_2)",subplot=2)
#text(log10.(N[i]),log10.(err2[i] ) ,"Euler") 
#
# Runge
#clear T Y err1 err2
s = 2
N = N0/s
err1 = zeros(length(N))
err2 = zeros(length(N))

for i=1:length(N)
    T,Y = ode_runge(phi_vdp,[t0 tf],y0,N[i])
    n=length(T)
    err1[i] = abs(y0[1]-Y[n,1])
    err2[i] = abs(y0[2]-Y[n,2])
end
#loglog(s*N,err1)
i=round(length(N)/2)
#text(s*N[i],err1[i] , "Runge")    
#loglog(s*N,err2)
#text(s*N[i],err2[i] , "Runge")  
Plots.plot!(log10.(s*N),log10.(err1),color="yellow", xlabel="log_{10}(fe)", ylabel="log_{10}(erreur pour y_1)",subplot=1)
i=round(length(N)/2)
#text(log10.(s*N[i]),log10.(err1[i] ) ,"Runge")
Plots.plot!(log10.(s*N),log10.(err2),color="yellow", xlabel="log_{10}(fe)", ylabel="log_{10}(erreur pour y_2)",subplot=2)
#     text(log10.(s*N[i]),log10.(err2[i] ) ,"Runge") 
# Heun
#clear T Y err1 err2
s = 3
N = N0/s
err1 = zeros(length(N))
err2 = zeros(length(N))

for i=1:length(N)
    T,Y = ode_heun(phi_vdp,[t0 tf],y0,N[i])
    n=length(T)
    err1[i]=abs(y0[1]-Y[n,1])
    err2[i]=abs(y0[2]-Y[n,2])
end
#loglog(s*N,err1)
i=round(length(N)/2)
#text(s*N[i],err1[i] , "Heun")    
#loglog(s*N,err2)
#text(s*N[i],err2[i] , "Heun")    
Plots.plot!(log10.(s*N),log10.(err1),color="magenta", xlabel="log_{10}(fe)", ylabel="log_{10}(erreur pour y_1)",subplot=1)
i=round(length(N)/2)
#text(log10.(s*N[i]),log10.(err1[i] ) ,"Heun")
Plots.plot!(log10.(s*N),log10.(err2),color="magenta", xlabel="log_{10}(fe)", ylabel="log_{10}(erreur pour y_2)",subplot=2)
#     text(log10.(s*N[i]),log10.(err2[i] ) ,"Heun") 

# RK4 classique
##clear T Y err1 err2
s = 4
N = N0/s
err1 = zeros(length(N))
err2 = zeros(length(N))

for i=1:length(N)
    T,Y = ode_rk4(phi_vdp,[t0 tf],y0,N[i])
    n=length(T)
    err1[i] = abs(y0[1]-Y[n,1])
    err2[i] = abs(y0[2]-Y[n,2])
end
#loglog(s*N,err1)
i=round(length(N)/2)
#text(s*N[i],err1[i] , "RK4")    
#loglog(s*N,err2)
#text(s*N[i],err2[i] , "RK4") 
Plots.plot!(log10.(s*N),log10.(err1),color="green", xlabel="log_{10}(fe)", ylabel="log_{10}(erreur pour y_1)",subplot=1)
i=round(length(N)/2)
#text(log10.(s*N[i]),log10.(err1[i] ) ,"Rk4")
Plots.plot!(log10.(s*N),log10.(err2),color="green", xlabel="log_{10}(fe)", ylabel="log_{10}(erreur pour y_2)",subplot=2)
#     text(log10.(s*N[i]),log10.(err2[i] ) ,"Rk4") 
# RK4 regle 3/8
#clear T Y err1 err2
s = 4
N = N0/s
err1 = zeros(length(N))
err2 = zeros(length(N))

for i=1:length(N)
    T,Y = ode_rk4(phi_vdp,[t0 tf],y0,N[i])
    n=length(T)
    err1[i]=abs(y0[1]-Y[n,1])
    err2[i]=abs(y0[2]-Y[n,2])
end
#loglog(s*N,err1)    
#loglog(s*N,err2)
Plots.plot!(log10.(s*N),log10.(err1),color="cyan", xlabel="log_{10}(fe)", ylabel="log_{10}(erreur pour y_1)",subplot=1)
i=round(length(N)/2)
Plots.plot!(log10.(s*N), log10.(err2),color="cyan", xlabel="log_{10}(fe)", ylabel="log_{10}(erreur pour y_2)",subplot=2)
# print -depsc fig1_ordre
# print -depsc fig2_ordre
#
# Gauss + point fixe
# -----
##clear T Y err1 err2
Nphie = []
s=4
N=N0/s
# options[2] = nbre max iteration point fixe
options[2] = 15
# epsilon pour le test du point fixe
# Il faut 1.e-12 car pour les valeurs de N grandes on atteint avec RK4 1.e-12 comme erreur absolue
options[3] = 1e-12
Ifail = []
err1 = zeros(length(N))
err2 = zeros(length(N))
Nphie = zeros(length(N))
for i=1:length(N)
    options[1] = N[i]
    T,Y,nphie,ifail = ode_gauss_v2(phi_vdp,[t0 tf],y0,options)
    if Ifail == []
        global Ifail = [length(findall(ifail==-1))] 
    else
        global Ifail = [Ifail length(findall(ifail==-1))] 
    end
    n=length(T)
    err1[i] = abs(y0[1]-Y[n,1])
    err2[i] = abs(y0[2]-Y[n,2])
    Nphie[i] = nphie
end
# Inutile de trie car deja en ordre
 # ---------------------
#loglog(s*N,err1,"red")    
#loglog(s*N,err2,"red")
#print -depsc fig_ordre_Gauss1
# vraie courbe
# ------------
#subplot(1,2,1)
Plots.plot!(log10.(Nphie),log10.(err1),color="red",xlabel="log_{10}(fe)",ylabel="log_{10}(erreur pour y_1)",subplot=1)
i=round(length(N)/2)
#text(log10.(Nphie[i] ) ,log10.(err1[i] ) ,"\color{red}Gauss")
#v=axis
#axis([v[1] v[2] v(3)-4 v(4)])
#subplot(1,2,2)
Plots.plot!(log10.(Nphie), log10.(err2),color= "red", xlabel="log_{10}(nphi)", ylabel="log_{10}(erreur pour y_2)",subplot=2)
i=round(length(N)/2)
#text(log10.(Nphie[i] ) ,log10.(err2[i] ) ,"\color{red}Gauss")
#axis([v[1] v[2] v(3)-4 v(4)])
##legend("Euler", "Runge","Heun","RK4_1","RK4_2","gauss v2 feps=1.e-12","Location","SouthWest")
#print -depsc fig_ordre_Gauss2
# vraie courbe
# ------------
#subplot(1,2,1)
Plots.plot!(log10.(Nphie),log10.(err1),color="red", xlabel="log_{10}(fe)", ylabel="log_{10}(erreur pour y_1)",subplot=1)
i=round(length(N)/2)
#text(log10.(Nphie[i] ) ,log10.(err1[i] ) ,"\color{red}Gauss")
#subplot(1,2,2)
Plots.plot!(log10.(Nphie),log10.(err2),color="red", xlabel="log_{10}(fe)", ylabel="log_{10}(erreur pour y_2)",subplot=2)
i=round(length(N)/2)
#text(log10.(Nphie[i] ) ,log10.(err2[i] ) ,"\color{red}Gauss")

#
# Gauss feps=1.e-6
#clear T Y err1 err2
Nphie = []
s = 4
N = N0/s
# options[2] = nbre max iteration point fixe
options[2] = 15
# epsilon pour le test du point fixe
# Il faut 1.e-12 car pour les valeurs de N grandes on atteint avec RK4 1.e-12 comme erreur absolue
options[3] = 1e-6
Ifail = []
err1 = zeros(length(N))
err2 = zeros(length(N))
Nphie = zeros(length(N))
for i=1:length(N)
    options[1] = N[i]
    T,Y,nphie,ifail = ode_gauss_v2(phi_vdp,[t0 tf],y0,options)
    if Ifail == []
        global Ifail = [length(findall(ifail==-1))] 
    else
        global Ifail = [Ifail length(findall(ifail==-1))] 
    end
    n=length(T)
    err1[i] = abs(y0[1]-Y[n,1])
    err2[i] = abs(y0[2]-Y[n,2])
    Nphie[i] = nphie
end
# 
# vraie courbe
# ------------
Plots.plot!(log10.(Nphie),log10.(err1), xlabel="log_{10}(fe)", ylabel="log_{10}(erreur pour y_1)",subplot=1)
i=round(length(N)/2)
#text(log10.(Nphie[i] ) ,log10.(err1[i] ) ,"\color{red}Gauss")
Plots.plot!(log10.(Nphie),log10.(err2), xlabel="log_{10}(fe)", ylabel="log_{10}(erreur pour y_2)",subplot=2)
i=round(length(N)/2)
#text(log10.(Nphie[i] ) ,log10.(err2[i] ) ,"\color{red}Gauss")
#
# Gauss fpitermax=2
#clear T Y err1 err2
Nphie = []
s = 4
N = N0/s
# options[2] = nbre max iteration point fixe
options[2] = 2
# epsilon pour le test du point fixe
# Il faut 1.e-12 car pour les valeurs de N grandes on atteint avec RK4 1.e-12 comme erreur absolue
options[3] = 1e-12
Ifail = []
err1 = zeros(length(N))
err2 = zeros(length(N))
Nphie = zeros(length(N))
for i=1:length(N)
    options[1] = N[i]
    T,Y,nphie,ifail = ode_gauss_v2(phi_vdp,[t0 tf],y0,options)
    if Ifail == []
        global Ifail = [length(findall(ifail==-1))] 
    else
        global Ifail = [Ifail length(findall(ifail==-1))] 
    end
    n=length(T)
    err1[i] = abs(y0[1]-Y[n,1])
    err2[i] = abs(y0[2]-Y[n,2])
    Nphie[i] = nphie
end
# vraie courbe
# ------------
#subplot(1,2,1)
Plots.plot!(log10.(Nphie),log10.(err1),color="green",xlabel="log_{10}(nphie)", ylabel="log_{10}(erreur pour y_1)",subplot=1)
i=round(length(N)/2)
#text(log10.(Nphie[i] ) ,log10.(err1[i] ) ,"\color{red}Gauss")
#v=axis
#axis([v[1] v[2] v(3)-2 v(4)])
#subplot(1,2,2)
Plots.plot!(log10.(Nphie),log10.(err2),color="green",xlabel="log_{10}(fnphi)", ylabel="log_{10}(erreur pour y_2)",subplot=2)
i=round(length(N)/2)
#text(log10.(Nphie[i] ) ,log10.(err2[i] ) ,"\color{red}Gauss")
#v = axis
#axis([v[1] v[2] v(3)-2 v(4)])
#legend("gauss v2 feps=1.e-12","gauss v2 feps=1.e-6","gauss v2 fpitermax=2","Location","SouthWest")
#print -depsc fig_ordre_Gauss3
#
# Gauss + Newton
# ---------------
Nphie = []
s=2
N=N0/(2*s)
# options[2] = nbre max iteration point fixe
options[2] = 15
# epsilon pour le test du point fixe
# Il faut 1.e-12 car pour les valeurs de N grandes on atteint avec RK4 1.e-12
# comme erreur absolue
options[3] = 1e-12
Ifail = []
err1 = zeros(length(N))
err2 = zeros(length(N))
Nphie = zeros(length(N))
for i=1:length(N)
    options[1] = N[i]
    T,Y,nphie,ndphie,ifail = ode_gauss_v3(phi_vdp,d_phi_vdp,[t0 tf],y0,options)
    if Ifail == []
        global Ifail = [length(findall(ifail==-1))] 
    else
        global Ifail = [Ifail length(findall(ifail==-1))] 
    end
    n=length(T)
    err1[i] = abs(y0[1]-Y[n,1])
    err2[i] = abs(y0[2]-Y[n,2])
    Nphie[i] = nphie+n*ndphie
end
# Inutile de trie car deja en ordre
# ---------------------
#plot(log10.(N0),log10.(err1),"red")   ,subplot=1 
##loglog(N0,err2,"red")
# vraie courbe
# ------------
#subplot(1,2,1)
Plots.plot!(log10.(Nphie),log10.(err1),color="red",ylabel="log_{10}(erreur pour y_1)",subplot=1)
i=round(length(N)/2)
#text(log10.(Nphie[i] ) ,log10.(err1[i] ) ,"\color{red}Gauss")
#xlabel("log_{10}(fe)")
##v=axis
##axis([v[1] v[2] v(3)-4 v(4)])
#subplot(1,2,2)
Plots.plot!(log10.(Nphie),log10.(err2),color="red",ylabel="log_{10}(erreur pour y_2)",subplot=2)
i=round(length(N)/2)
#text(log10.(Nphie[i] ) ,log10.(err2[i] ) ,"\color{red}Gauss")
#xlabel("log_{10}(nphi)")
##axis([v[1] v[2] v(3)-4 v(4)])
##legend("Euler", "Runge","Heun","RK4_1","RK4_2","gauss v2 feps=1.e-12","Location","SouthWest")
display(plt)